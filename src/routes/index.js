...
const { ROUNDED_FIELDS } = process.env;
...
const formatResult = ({ outputFormat, rawData }) => {
  ...
  if (!rawData || !rawData.hasOwnProperty("metaData")) {
    ...
    throw new Error(
      `Raw data has not the mandatory field: {"metaData:": [ ... ], ...}`
    );
  }
  ...
  if (
    !rawData ||
    !rawData.hasOwnProperty("rows") ||
    !Array.isArray(rawData.rows)
  ) {
    console.error(rawData);
    throw new Error(
      `Raw data has not the mandatory field: {"rows:": [ ... ], ...}`
    );
  }
  ...
  const dimention2index = rawData.metaData.reduce(
...
const postProcessResults = ({ result }) => {
  // Round fields post-processing
  if (typeof ROUNDED_FIELDS === "string") {
    const split = ROUNDED_FIELDS.split(",");

    const field2round = split.map(f => {
      const [field, decimals] = f.split(":");
      return { field, decimals };
    });

    // Apply only if field2round is valid
    // Result shape: {result: {entry: [...]}}
    if (
      field2round.length &&
      result.result &&
      Array.isArray(result.result.entry)
    ) {
      result.result.entry = result.result.entry.map(i => {
        // Clone result set item
        let processed = Object.assign({}, i);
        // Loop on fields to round
        field2round.forEach(({ field, decimals }) => {
          // if value is numeric, apply rounding
          if (!isNaN(processed[field])) {
            const castedToFloat = parseFloat(processed[field]);
            processed[field] = `${castedToFloat.toFixed(decimals)}`; // cast back to string
          }
        });
...
config.forEach(rawConfig => {
  ...
  router[method.toLowerCase()](normalizePath(path), async (req, res, next) => {
    ...
    try {
      ...
      const finalResult = postProcessResults({ result: formattedResult });

      res.json(cloneDeep(finalResult));
    } catch (err) {
